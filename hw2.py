...
def courses_long(courses, mentors, durations):
    courses_list = []
    ...
    durations_dict = {}
    for id, z in enumerate(courses_list):
    	key = z["duration"]
    	if key in durations_dict:
    		durations_dict_list = durations_dict[key]
    		durations_dict_list.append(id)
    		durations_dict[key] = durations_dict_list
    	else:
    ...
    for key, val in durations_dict.items():
    	for c in val:
    		print(f'{courses_list[c]["title"]} - {key} месяцев')
...
print(courses_long(courses, mentors, durations))